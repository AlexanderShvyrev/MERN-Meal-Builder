...
const OneProduct = (props) => {
  ...
  const createNew = (e) => {
    const newProduct = {
      addedName: newName,
      addedPrice: price,
      addedIngredients: ings,
      cust_id: userId,
    };
    axios
      .post('http://localhost:8000/api/addedProducts', newProduct)
      .then((res) => {
        console.log(res);
    ...
    if (orders.length < 1) {
      ...
      for (var i = 0; i < orders.length; i++) {
        if (userId === orders[i].customer) {
          ...
          const updatedProd = {
            totalPrice: newProduct.addedPrice + orders[i].totalPrice,
            products: [...orders[i].products, newProduct],
          };
          console.log('in the else to create order');
          axios
            .put(
              `http://localhost:8000/api/orders/${orders[i].customer}`,
              updatedProd
            )
            .then((res) => {
              console.log(res);
      ...
      {product ? (
        <div className='card text-center bananas'>
          ...
          <div className='card-body'>
            <h5 className='card-text ' style={{ color: '#BD6026' }}>
              {' '}
              Price: ${price.toFixed(2)}
            </h5>
            <div
              className='originalzone'
